...
3. **確定済み DP を読み取り、生成物を分岐する**（B-23 / Issue #182）。
   ...
   - DP-AUTHFLOW-001 が `undecided` の場合は **seeds を確定生成せず**、`docs/pending-decisions.md` に下記フォーマットで追記する（warn_and_document, T-002）:
     ```markdown
     | <YYYY-MM-DD> | DP-AUTHFLOW-001 | 運用ユーザー作成ポリシー（invitation_based / self_signup）未確定のため `db/seeds.rb` を生成しなかった。確定後に implementer を再実行すること。 | implementer | implementer | 未決 |
     ```
     列順は本ファイルの「未決リスト」表ヘッダ（起票日 / 仮ID / 概要 / 関連タスク / 起票者 / 状態）に揃える。
