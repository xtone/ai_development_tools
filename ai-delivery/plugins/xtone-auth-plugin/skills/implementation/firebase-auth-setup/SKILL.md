...
3. **トークン失効 / リフレッシュ（2 段階の失効）**: ID トークン短命＋リフレッシュトークン。サーバ側失効は **2 段階に分ける**:
   - **hard 失効**（退会・パスワード変更・不正検知）: サーバ側 `tokens_valid_after = now` ＋ IaaS の refresh 失効。以後 `auth_time < tokens_valid_after` のトークンを即拒否。
   - **soft 失効**（**MFA 変更**）: IaaS の refresh のみ失効（他デバイスの古い MFA 無しトークンを無効化）。**サーバ側 `tokens_valid_after` は触らない**。Firebase の `auth_time` クレームは MFA enrollment では更新されないため、即拒否すると enroll 直後の同セッションが `token revoked`（401）になる。
