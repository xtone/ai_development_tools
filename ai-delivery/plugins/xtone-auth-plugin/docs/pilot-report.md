...
| F-5 | Med | **MFA（DP-008）の実装ガイドが無い**。design で「オプトイン／管理者必須」と決めたが、Firebase での MFA 実装手順がスキルに無い | B-04 |
| F-6 | Med | **成果物のスキーマ検証手段が無い**（TPL-27 validate-plugin.sh 未整備）。delivery/*.json をスキーマ検証できず手動確認 | B-07 |
| F-7 | Low | **退会済みアカウントの再登録ポリシーが判断ポイント化されていない**。実装中に「退会後の同一 UID 再ログインを許すか」を判断する必要が生じた（本実装は 403 で拒否）。DP に無い | B-08 |
...
## 5. Rollout 推進可否判定（たたき台）

**判定案: 条件付き Go（Conditional Go）**

- **根拠**: 型としての中核価値（一貫フロー・判断ポイント追跡・差し替え可能設計・warn_and_document）が **実アプリ構築で実用に耐えると実証**された。サンプルアプリは全テスト通過し、撤退基準に該当しない。
- **条件**: Rollout 着手前に backlog の **High 項目（B-01 / B-02 / B-05 / B-06）** を解消すること。これらは「型の穴」であり、24 ユースケース展開で繰り返しコスト化する。
- **次アクション**: 豊田さんによる本書レビュー → フィードバック反映 → Rollout go/no-go の確定。
