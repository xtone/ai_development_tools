...
> - 残 **B-07**（Med）は **Rollout 並行**で順次対応。**B-08 は DP-28 として正式起票で CLOSED**、**B-09 は対応済**（Rails AuthAdapter テンプレ同梱 / Issue #134）。
...
| ~~**B-09**~~ ✅ | [#134](https://github.com/xtone/ai_development_tools/issues/134) | Plugin（任意） | Low | ~~**言語別の実装テンプレ**（例: Rails 向け AuthAdapter コード雛形）をスキルに同梱し実装を加速~~ → **Rails 向けテンプレ同梱**（`firebase-auth-setup/templates/rails/`：adapter / firebase_adapter / test_adapter / authenticatable / app_auth initializer / migration / Gemfile.snippet / dotenv.sample / README）。SKILL.md と references/rails.md に導線追加 | SKL- | F-8 |
...
| **B-13** | [#153](https://github.com/xtone/ai_development_tools/issues/153) | Architecture | High | **実装フェーズに skill_plan 生成を強制**。`implementation-skill-planner` スキル新設＋ `design.schema.page_access_control` / `local_dev_stack` 追加＋ `implementation-plan.schema.skill_plan` 追加（**required かつ minItems=1**。MVP 期は CONV-14 並行保持を厳格適用せず最新スキーマで型化を強制 / DP-MVP-COMPAT）。frontend / emulator スキル呼び出し漏れ防止。**B-18 で `xtone-shared-plugin/skills/implementation/implementation-skill-planner/` に横断化（auth-plugin / template から symlink）** | SKL- / FLD- | サンプル案件 sample-auth |
...
| ~~**B-11**~~ ✅ | [#156](https://github.com/xtone/ai_development_tools/issues/156) | Tooling | High | **`tech-version-check` スキル新設**。context7 / WebFetch で公式最新安定版・互換性を事前取得し `delivery/version-matrix.md` に記録。Ruby/Rails 等の非互換に途中で当たる事故を防止。**B-17 で `xtone-shared-plugin/skills/implementation/tech-version-check/` に横断化（auth-plugin / template から symlink）** | SKL- / TPL- | sample-auth |
| **B-17** | [#164](https://github.com/xtone/ai_development_tools/issues/164) | Architecture | Med | **`tech-version-check` を横断スキル化**。B-11 で暫定的に auth-plugin 内に配置していた本スキルを `xtone-shared-plugin/skills/implementation/tech-version-check/` に移管し、`xtone-plugin-template` と各プラグインからは symlink で参照（CONV-14 SSoT） | SKL- / CONV-14 | PR #162（B-11）マージ後 |
...
| **B-18** | [#171](https://github.com/xtone/ai_development_tools/issues/171) | Architecture | High | **`implementation-skill-planner` を横断スキル化**。B-13 で auth-plugin 内に暫定配置していた本スキルを `xtone-shared-plugin/skills/implementation/implementation-skill-planner/` に移管し、`xtone-plugin-template` と各プラグインからは symlink で参照（CONV-14 SSoT）。Rollout（T-023〜T-045）展開前準備 P0 | SKL- / CONV-14 | PR #168（B-17）方式 |
...
- **Low（任意・改善）**: ~~B-08~~ ✅（DP-28 で起票済） / ~~B-09~~ ✅（Rails テンプレ同梱） / B-16。
