...
プラグイン実装・サンプルアプリ構築の **実行環境前提** と **バージョン選定方針**。T-022 パイロット発見 F-1（想定環境の Ruby が古く最新 Rails が動かない / 環境前提が未文書化）の解消（Issue #129）。

## バージョン方針（重要）

1. **バージョンは固定しない（陳腐化回避）。** ドキュメント・テンプレートに特定の数値（例: 「Ruby 3.x」「Rails 8」）をハードコードしない。
2. **基本は公式の最新安定版を使う。** 着手時に下記「公式情報源」から最新安定版を確認して導入する。
3. **特定バージョンが必要な場合は判断ポイント。** クライアント制約・レガシー互換・特定ライブラリ依存などで最新以外を使う必要があるときは、**人間に確認**し、決定を `docs/pending-decisions.md` → 判断ポイントカタログDB（DP-）に記録する（T-002 warn_and_document）。AI が勝手に固定しない。
...
T-022 では想定環境のシステム Ruby が古く（例: 2.6 系）、最新 Rails が動かなかった。`rbenv` で当時の最新安定版（3.3 系）を導入して回避した。**着手時に公式最新を用意する**ことを徹底する。なお、当時の具体的な数値は「その時点の最新」であって将来の固定要件ではない。

## 関連

- 親ガイド: `../CLAUDE.md` / `README.md`
- プラグイン実装の言語別レシピ: `plugins/<plugin>/skills/implementation/<skill>/references/<stack>.md`
- 判断ポイント（特定バージョン固定が必要な場合の起票先）: `docs/pending-decisions.md` / 判断ポイントカタログDB
