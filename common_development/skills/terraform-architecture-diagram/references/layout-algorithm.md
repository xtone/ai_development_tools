...
**重要**: [drawio-xml-guide.md §エッジラベル付与の判定基準](drawio-xml-guide.md) に従いエッジラベルを最小化するため、**凡例が線種の意味判別の主要手段**となる。凡例が不完全だと読み手は線の意味を判別不能になるため、**ダイアグラムで使用している全線色・全線種を漏れなく列挙すること**。

**必須要件**:
- **タイトルは日本語「凡例」を使用**（`Legend` のような英単語は外部読者に伝わりづらいため禁止）
- ダイアグラムに実際に使われている**全ての色・線種を漏れなく**列挙（一つでも欠けると意味判別不能）
- 注釈スタイル（`fillColor=#ffe6cc`）で背景を付ける
- **配置位置**: 図全体下部または左下、視線がアクセスしやすい場所
