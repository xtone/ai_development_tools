...
| 項目 | 値 |
|------|-----|
| **入力ファイル** | `.artifacts/01_specification.json`, `.artifacts/03_usecases.json` |
| **出力ファイル** | `.artifacts/04_openapi.yaml`, `docs/api/openapi.yaml` |
| **依存ステップ** | ステップ1, ステップ3 |

### 出力ファイル形式

OpenAPI 3.1形式のYAMLファイル。詳細は手順セクションを参照。
...
- 定義したスキーマ数
- 出力ファイルパス
- バリデーション結果（`npx @redocly/cli lint` の結果）
